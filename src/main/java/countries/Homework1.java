...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        return countries.stream().anyMatch(c->c.getName().toLowerCase().contains("island"));
    }
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream().map(Country::getName).filter(d->d.toLowerCase().contains("island")).findFirst();
    }
    ...
    public void streamPipeline3() {
        countries.stream().map(Country::getName).filter(d->d.toLowerCase().charAt(0)==d.toLowerCase().charAt(d.length()-1)).forEach(System.out::println);
    }
    ...
    public void streamPipeline8() {
        countries.stream().sorted(Comparator.comparingInt(c->c.getTimezones().size())).map(d->d.getName()+":"+d.getTimezones().size()).forEach(System.out::println);
    }
...
ed by the language code "es").
    ...
    public long streamPipeline9() {
        return countries.stream().filter(c->!c.getTranslations().containsKey("es")).count();
    }
    ...
    public void streamPipeline11() {
        countries.stream().flatMap(c->c.getTranslations().keySet().stream()).distinct().sorted().forEach(System.out::println);
    }
    ...
    public void streamPipeline13() {
        countries.stream().filter(c->c.getArea()==null).map(Country::getRegion).distinct().forEach(System.out::println);
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(c->c.getArea()!=null && (c.getArea().compareTo(BigDecimal.ONE)) < 0).map(Country::getName).forEach(System.out::println);
    }
    ...
    public void streamPipeline16() {
    	countries.stream().filter(c->c.getRegion()==Region.ASIA||c.getRegion()==Region.EUROPE).flatMap(c->c.getTimezones().stream()).distinct().forEach(System.out::println);
    }
